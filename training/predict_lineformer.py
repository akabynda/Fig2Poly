...
def main() -> int:
    ...
    model = init_detector(
        str(root / "lineformer_swin_t_config.py"), str(args.weights), device="cuda:0"
    )
    output = args.output.resolve()
    output.mkdir(parents=True, exist_ok=True)
    images = sorted(
        p for p in args.input.resolve().iterdir()
    ...
    for image_path in images:
        ...
        for index, (score, mask, bbox) in enumerate(selected, 1):
            ...
            cv2.imwrite(str(item_dir / mask_name), mask.astype(np.uint8) * 255)
            contours, _ = cv2.findContours(
                mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
            cv2.drawContours(overlay, contours, -1, color.tolist(), 1, cv2.LINE_AA)
            cv2.putText(
                overlay, f"{index}: {score:.2f}",
                (int(bbox[0]), max(12, int(bbox[1]))),
                cv2.FONT_HERSHEY_SIMPLEX, 0.42, color.tolist(), 1, cv2.LINE_AA,
            )
            records.append(
                {"id": index, "score": score, "bbox_xyxy": bbox, "mask": mask_name}
            )
        ...
        cv2.imwrite(str(item_dir / "instance_ids.png"), instance_ids)
